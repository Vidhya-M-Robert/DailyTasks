function findPowerSum(num: number): string {
    if(Math.round(num) !== num || num<=0 || typeof(num) !== "number"){
        return "Invalid"
    }
    let sum: bigint = 0n;

    for (let i: bigint = 1n; i <= BigInt(num); i++) {
        let power: bigint = i ** i;
        sum += power;
    }

    const stringValue: string = sum.toString();
    const lastTen: string = stringValue.slice(-10).padStart(10, "0");
